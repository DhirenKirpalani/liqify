...
export default function TradingChart({ market }: TradingChartProps) {
  ...
  // Fetch market data when market or timeframe changes
  useEffect(() => {
    // Only fetch data if API calls are available
    if (!getMarketHistory || !getMarketPrice) return;
    ...
    const fetchData = async () => {
      ...
      if (isRequestInProgress || !isMounted) return;
      ...
      try {
        ...
        // Get the latest price with slight delay to avoid request bursts
        if (isMounted) {
          const price = await getMarketPrice(market);
          setCurrentPrice(price);
        }
    ...
    // Set up interval to refresh data - using a longer interval (30 seconds)
  ...
  }, [market, timeframe, getMarketHistory, getMarketPrice, isMounted, chartData.length]);
